...
To explore the spatial distribution of transposable elements across the genome, we generated a circos-style TE density plot using the R package circlize. 
...
## Part 3.4: Processing the GENESPACE Pangenome Matrix

This step takes the orthogroup and synteny-aware output generated by GENESPACE and produces a cleaned, analysis-ready pangenome matrix. The script
`Scripts/03.4-process_pangenome.sh` automates the post-processing of GENESPACE results by preparing the `pangenome_matrix.rds` file for downstream pangenome and presence–absence analyses


Outputs

- `pangenome_matrix.cleaned.rds`
  Cleaned, synteny-filtered pangenome matrix.
- `presence_table.tsv`
  Orthogroup-level presence/absence table.
- `gene_counts_per_genome.tsv`
  Summary of core, accessory and accession-specific genes per genome.
- `pangenome_frequency_plot.pdf`
  Bar plot of orthogroup/gene frequencies across genomes.
- `freq_summary.tsv`
  Final summary table used in the report.

To run the script, the following command is used : 

```
sbatch Scripts/03.4-process_pangenome.sh
```
